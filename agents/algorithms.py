...
class PolicyGradient:
    ...
    def _get_episode_length(env):
        """Infer the number of timesteps in one environment episode."""
    ...
    def _get_portfolio_size(env):
        """Infer the number of portfolio weights produced by the policy."""
        ...
        for episode in pbar:
            ...
            while not done:
                ...
                if getattr(self.train_env, "asset_memory", None):
                    final_value = self.train_env.asset_memory[-1]
                ...
                if len(self.train_buffer) == self.train_batch_size:
                    last_loss = self._gradient_ascent()

                obs = next_obs

            # gradient ascent with episode remaining buffer data
            last_loss = self._gradient_ascent() if len(self.train_buffer) > 0 else last_loss
